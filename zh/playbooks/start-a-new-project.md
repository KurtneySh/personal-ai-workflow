...
- Level 1 项目可在 `README.md` 或 `STATE.md` 写清验证要求、禁止操作和完成标准。
- Level 2+ 项目应在 `AGENTS.md` 写清验证要求、禁止操作和完成标准。
...
- Action: 检查真实项目文件、脚本和工具链；如果存在真实命令，只自动运行 read-only 或低风险本地最小必要验证命令；如果命令可能部署、migrate 数据、写文件、安装依赖、联系外部服务、改变认证/权限，或触碰生产/敏感数据，先停下来请求人类确认；在得到人类确认后，Level 1 把命令和结果写入 `README.md` 或 `STATE.md`，Level 2+ 按项目约定写入 `README.md` / `STATE.md` / `AGENTS.md` 中合适的位置；如果 `AGENTS.md` 不存在，不要假设它存在。
...
- 为什么这是最小必要验证命令
- 实际运行结果
- 建议写入位置和内容：Level 1 写入 `README.md` 或 `STATE.md`；Level 2+ 写入 `README.md` / `STATE.md` / `AGENTS.md` 中合适的位置
...
只有在人类确认后，才把命令和结果写入文件；Level 1 写入 `README.md` 或 `STATE.md`，Level 2+ 写入 `README.md` / `STATE.md` / `AGENTS.md` 中合适的位置。如果 `AGENTS.md` 不存在，不要假设它存在或无条件创建。确认前请明确说明“等待人类确认后再编辑文件”。
...
- Input: README、STATE，以及如果存在的 AGENTS 中记录的验证命令。
...
```text
请检查 README、STATE，以及如果存在的 AGENTS 中记录的验证命令，并运行最小必要验证。
